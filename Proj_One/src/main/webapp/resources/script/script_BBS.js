...
/*	$("#writeBtn").click(function(){		
	...
	$("#writeBtn").click(function(){
		$("#zz").attr("action","/bbs/write");
		$("#zz").submit();
	});
	
	/* 리스트 페이지 글쓰기 버튼 끝 /bbs/list */
	
	
	/* 글쓰기 페이지 게시글 등록 시작 /bbs/write */
	...
	$("td.update>button#modProcBtn").click(function(){
		...
		if (subject == "") {
			...
			//return;
			$("#modifyFrm").attr("action","/bbs/modify");
			$("#modifyFrm").submit();
		}
	...
	$("td.reply>button#replyBtn").click(function(){
		...
		if (subject == "") {
			...
			$("#replyFrm").attr("action", "/bbs/reply");
			$("#replyFrm").submit();
		}
