		...
		<main id="main" class="dFlex">
			...
			<div id="contents" class="bbsList">
				...
				<c:if test="${empty keyWord}">
					<c:set var="prnType" value="전체 회원수" />
				</c:if>
				<c:if test="${!empty keyWord }">
				...
				<div id="pageInfo" class="dFlex">
					<span>${prnType } : ${totalRecord } 명</span> <span>페이지 :
								...
								<form name="searchFrm" class="dFlex" id="searchFrm">
									<div>
										<select name="keyField" id="keyField" value="${keyField }">
											<option value="uId"
												<c:if test="${keyField eq 'uId' }">selected="selected"</c:if>>아이디</option>
											<option value="uName"
												<c:if test="${keyField eq 'uName' }">selected="selected"</c:if>>이름</option>
										</select>
									</div>
									<div>
								...
								%> <c:if test="${totalPage != 0 }">
									...
									<c:if test="${nowBlock > 1 }">
										<span class="moveBlockArea"
											onclick="moveBlock1('${nowBlock-1 }', '${blockSize }')">
											&lt; </span>
									</c:if>
									<c:if test="${nowBlock <= 1 }">
									...
									<c:forEach var="i" begin="${firstPage}" end="${totalPage}">
										...
										<c:if test="${i != nowPage }">
											<span class="pageNum" onclick="movePage1('${i}')"> ${i }
											</span>
										</c:if>
									</c:forEach>
