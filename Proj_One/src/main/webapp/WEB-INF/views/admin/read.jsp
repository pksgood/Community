	...
	<div id="wrap">
		...
		<main id="main" class="dFlex">
			...
			<div id="contents" class="bbsRead">
					...
					<tfoot id="readTblFoot">
						...
						<tr>
							...
							<td colspan="4" id="btnAreaTd" class="read">
								<button type="button" id="listBtn1"><%=listBtnLabel %></button>
								<c:if test="${!empty uId_Session }">
									<c:if test="${uId_Session == data.uId }">
										<button type="button" id="delBtn1">삭 제</button>
										<button type="button" id="modBtn1">수 정</button>
									</c:if>
								</c:if>
				...
				<c:import url="/admin/comments" />
				<c:import url="/admin/commentsList" />
			</div>
			<!-- 실제 작업 영역 끝 -->
